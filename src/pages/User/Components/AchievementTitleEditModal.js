 ...
 * One-step modal to edit the "Achievement Title" (type=9) background (image and colorCode).
 *
 * @param {boolean} show - Whether to show the modal
 * @param {function} onClose - Function to close the modal
 * @param {object} inUseAchievementTitle - The currently in-use achievement title detail (type=9)
 * @param {function} setTempAchievementTitle - Optional function to preview the new achievement title in the parent
 */
const AchievementTitleEditModal = ({
  show,
  onClose,
  inUseAchievementTitle = {},
  setTempAchievementTitle,
}) => {
  const [themes, setThemes] = useState([]); // fetched themes for type=9
  const [selectedTitleId, setSelectedTitleId] = useState(inUseAchievementTitle.id || null);
  const [showConfirm, setShowConfirm] = useState(false);
  const [originalTitle, setOriginalTitle] = useState(inUseAchievementTitle);

  // When modal opens or when parent's inUseAchievementTitle changes, update local state
  useEffect(() => {
    if (show) {
      setOriginalTitle(inUseAchievementTitle);
      setSelectedTitleId(inUseAchievementTitle.id);
      fetchAchievementTitleThemes();
    }
  }, [show, inUseAchievementTitle.id]);

  const fetchAchievementTitleThemes = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Lỗi khi fetch Achievement Title!");
      }
      ...
      console.error(error);
      message.error("Không thể tải Achievement Title!");
    }
  };
  ...
  const handleConfirmApiCall = async () => {
    ...
    const previousDetailId = inUseAchievementTitle.id;
  ...
  const handleClose = () => {
    if (setTempAchievementTitle && originalTitle) {
      setTempAchievementTitle({
...
export default AchievementTitleEditModal;
